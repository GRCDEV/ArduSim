...
There are another two differences compared to a "*UAV agent*". First, a set of pictures can be downloaded from Google Static Maps to integrate the background image, which is geopositioned on the theoretical location of the virtual UAVs. Second, storing the experiment results is optional, while in a real UAV it is always accomplished. Sadly, Google has stopped serving the background images for free, so the background map will not be available until a new version of ArduSim is released. We are planning to give support to Bing images, and also give the user the chance to use a paid key to utilize the current implementation for Google Static Maps API.
...
* `void openConfigurationDialog()`. It opens a dialog implemented by the protocol developer, and that allows the user to input parameters related to the protocol. When the dialog is accepted, please remember to issue the command `API.getArduSim().setProtocolConfigured(true)` to force the simulator to open the main window. If the developer doesn't want a dialog, then it is enough to call the forementioned function.
...
* `void startThreads()`. This method is used to start the threads used by the protocol. Then, ArduSim waits the user to press the setup button. In general, the threads must use methods included in `API.getArduSim()` object to wait until the setup or start button is pressed before performing any action (see section "[5.4 ArduSim integration](#markdown-header-54-ardusim-integration)").
...
* `void startExperimentActionPeformed()`. It could be useful to perform the take off, or to start a mission from the ground. If the protocol needs to perform additional actions over the UAV, it is recommended to do it in the protocol threads to free this thread and allow it to check periodically if the experiment finishes. In any case, you can always perform the actions directly in the protocol threads. If you use this method to perform the take off, the protocol threads should wait until a shared variable is changed when the take off process finishes. Nevertheless, while following a mission, the control over the multicopter is lost immediately after the mission start, so the protocol must have its own logic to decide when to take control over the multicopter.
...
* `void setWaypointReachedListener(WaypointReachedListener)`. Any Class can implement *WaypointReachedListener.java*, as *mbcap.logic.BeaconingThread* does. Then, using this method, that Class would be able to apply some logic each time the flight controller detects that a waypoint has been reached.
