...
import static fme.pojo.State.SETUP_FINISHED;
...
import fme.gui.FMeConfigDialog;
...
public class FMeHelper extends ProtocolHelper {
	...
	public void initializeDataStructures() {
		...
		FMeParam.state = new AtomicIntegerArray(numUAVs);	// Implicit value State.START, as it is equals to 0
		
		FMeParam.data = new AtomicReference<>();
		FMeParam.idNext = new AtomicLongArray(numUAVs);
		FMeParam.flyingFormation = new AtomicReferenceArray<>(numUAVs);
	}

	@Override
	public String setInitialState() {
		return FMeText.START;
	}
	...
	public Pair<GeoCoordinates, Double>[] setStartingLocation() {
		Location2D masterLocation = Location2D.NewLocation(FMeParam.masterInitialLatitude, FMeParam.masterInitialLongitude);
		...
		double yawRad = FMeParam.masterInitialYaw;
		double yawDeg = yawRad * 180 / Math.PI;
		UTMCoordinates offsetMasterToCenterUAV = groundFormation.getOffset(FMeParam.MASTER_POSITION, yawRad);
		for (int i = 0; i < numUAVs; i++) {
			...
			if (i == FMeParam.SIMULATION_MASTER_ID) {
	...
	public boolean sendInitialConfiguration(int numUAV) {
		...
		if (FMeHelper.isMaster(numUAV)) {
			if (!Copter.setParameter(numUAV, ControllerParam.LOITER_SPEED_357, FMeParam.masterSpeed)) {
				return false;
			}
		}
		return true;
	...
	public void setupActionPerformed() {
		...
		while (!allFinished) {
			...
			for (int i = 0; i < numUAVs && allFinished; i++) {
				if (FMeParam.state.get(i) < SETUP_FINISHED) {
					allFinished = false;
				}
			}
			if (!allFinished) {
	...
	public void startExperimentActionPerformed() {
		new RemoteThread(FMeParam.MASTER_POSITION).start();
	}
	...
	public static boolean isMaster(int numUAV) {
		...
		if (role == Tools.MULTICOPTER) {
			...
			long idMaster = Tools.getIdFromPos(FMeParam.MASTER_POSITION);
			for (int i = 0; i < FMeParam.MAC_ID.length; i++) {
				if (FMeParam.MAC_ID[i] == idMaster) {
					return true;
				}
			}
		} else if (role == Tools.SIMULATOR) {
			if (numUAV == FMeParam.MASTER_POSITION) {
				return true;
			}
		}
		return b;
