...
public class BoardHelper {
	...
	public static void downloadBackground() {
		...
		try {
			...
			Location2DGeo boardLeftUpGeo = Location2DUTM.getGeo(boardUpLeftUTMX, boardUpLeftUTMY);
			Location2DGeo boardRightUpGeo = Location2DUTM.getGeo(boardUpRightUTMX, boardUpRightUTMY);
			Location2DGeo boardLeftBottomGeo = Location2DUTM.getGeo(boardDownLeftUTMX, boardDownLeftUTMY);
			Location2DGeo boardRightBottomGeo = Location2DUTM.getGeo(boardDownRightUTMX, boardDownRightUTMY);
			...
			Location2DUTM upLeftUTMCorner = Location2DGeo.getUTM(maxLatitude, minLongitude);
			Location2DUTM upRightUTMCorner = Location2DGeo.getUTM(maxLatitude, maxLongitude);
			Location2DUTM downLeftUTMCorner = Location2DGeo.getUTM(minLatitude, minLongitude);
			...
			Location2DGeo tile1GeoCenter = Location2DUTM.getGeo(tile1UTMCenterX, tile1UTMCenterY);
			...
			for (int i=0; i<BoardParam.map.length; i++) {
				for (int j=0; j<BoardParam.map[i].length; j++) {
					...
					tileCenter = Location2DUTM.getGeo(UTMx, UTMy);
	...
	private static int getMapZoom(double latitude, double longitude,
		...
		while (zoom>0 && !scaleFound) {
			projection = new MercatorProjection(
					...
					pxHeight,
					Math.pow(2, zoom));
			Location2DGeo upLeftGeo = projection.getGeoLocation(0, 0);
			Location2DUTM upLeftUTM = upLeftGeo.getUTM();
			Location2DGeo bottomRightGeo = projection.getGeoLocation(pxWidth, pxHeight);
			Location2DUTM bottomRightUTM = bottomRightGeo.getUTM();
			double width, height;
	...
	public static void paintBoard(Graphics g, BoardPanel p) {
		...
		if (!BoardParam.drawAll) {
			...
			if (numUAVsConnected > BoardParam.numUAVsDrawn.get()) {
				...
				ArduSimTools.selectedProtocolInstance.rescaleDataStructures();
				BoardHelper.rescaleUAVPath();
				BoardHelper.rescaleMissions();
				ArduSimTools.selectedProtocolInstance.rescaleShownResources();
	...
	private static void rescaleMissions() {
		...
		Point2D.Double locationPX;
		Location3DUTM prevWaypoint, nextWaypoint;
		if (UAVParam.missionUTMSimplified != null) {
			for (int i=0; i<Param.numUAVs; i++) {
				...
				if (mission != null && mission.size() > 0) {
					...
					locationPX = BoardHelper.locatePoint(locationUTM.x, locationUTM.y);
					prevWaypoint = new Location3DUTM(locationPX.x, locationPX.y, locationUTM.z);
					for (int j=1; j<mission.size(); j++) {
						locationUTM = mission.get(j);
						locationPX = BoardHelper.locatePoint(locationUTM.x, locationUTM.y);
						nextWaypoint = new Location3DUTM(locationPX.x, locationPX.y, locationUTM.z);
	...
	private static void drawAll(Graphics2D g, BoardPanel p) {
		...
		ArduSimTools.selectedProtocolInstance.drawResources(g, p);
