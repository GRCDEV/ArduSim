...
public class CommLinkObject {
	...
	public static final int RANGE_CHECK_PERIOD = 1000;				// (ms) Time between UAVs range check
	...
	private static final int MESSAGE_WAITING_TIME = 1;				// (ms) Waiting time to check if a message arrives
	...
	public CommLinkObject(int numUAVs, int port) {
		...
		if (Param.role == Tools.SIMULATOR) {
			...
			try {
				...
				sendPacket = new DatagramPacket(new byte[Tools.DATAGRAM_MAX_LENGTH],
						Tools.DATAGRAM_MAX_LENGTH,
						InetAddress.getByName(UAVParam.broadcastIP),
						port);
				receiveSocket = new DatagramSocket(port);
				receiveSocket.setBroadcast(true);
				receivePacket = new DatagramPacket(new byte[Tools.DATAGRAM_MAX_LENGTH], Tools.DATAGRAM_MAX_LENGTH);
		...
		if (Param.role == Tools.SIMULATOR) {
			...
			if (prevMessage != null) {
				...
				while (prevMessage.end > now) {
					Tools.waiting(MESSAGE_WAITING_TIME);
					if (!messageWaited) {
						...
						if (prevMessageOther == null || prevMessageOther.end <= now) {
							if (CommLinkObject.pCollisionEnabled) {
								if (vBufferUsedSpace.get(i) + message.length <= receivingvBufferSize) {
									vBuffer[i].add(sendingMessage.clone());
								...
								if (mBuffer[i].offerLast(sendingMessage.clone())) {
	...
	public byte[] receiveMessage(int numUAV, int socketTimeout) {
		if (Param.role == Tools.SIMULATOR) {
			...
			receivePacket.setData(new byte[Tools.DATAGRAM_MAX_LENGTH], 0, Tools.DATAGRAM_MAX_LENGTH);
			try {
				if (socketTimeout > 0) {
