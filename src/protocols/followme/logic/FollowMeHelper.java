...
import protocols.followme.gui.FollowMeConfigDialogApp;
import protocols.followme.gui.FollowmeSimProperties;
...
import static protocols.followme.pojo.State.SETUP_FINISHED;
		...
		if (copter.getMasterSlaveHelper().isMaster()) {
			...
			if(version.contains("3.5")) {
				...
				API.getGUI(0).log("this version is not checked yet,"
						...
						+ " src:protocols.followme:followmeHelper.java:sendInitialConfiguration");
				return copter.setParameter(CopterParam.LOITER_SPEED_36X, FollowMeParam.masterSpeed);
			}
		}
		return true;
