package protocols.muscop.logic;
...
public class MUSCOPHelper extends ProtocolHelper {
	...
	public boolean loadMission() {
		// In a real multicopter, the UAV is always in position 0. The master UAV is which has a protocols.mission to load.
	...
	public Pair<Location2DGeo, Double>[] setStartingLocation() {
		...
		// The ground formation will appear with the UAV in the center in the first waypoint of the protocols.mission from the master
		// The protocols.mission to be followed will start in the location where the flight center UAV will be after taking off
		// We overlap the original master protocols.mission with the center UAV protocols.mission for visualization
		
		// 1. Get the yaw of the master given the current protocols.mission
		//    We only can set yaw if at least two points with valid coordinates are found
		// 1.1. Check if master protocols.mission exists
		...
		List<Waypoint>[] missions = missionHelper.getMissionsLoaded();	// Simplified protocols.mission would be better, but it is not ready jet
		...
		// 2. With the ground formation centered on the protocols.mission beginning, get ground coordinates.
		//   The UAVs appear with the center UAV in the first waypoint of the initial protocols.mission.
		...
		//   First, we copy the protocols.mission as is
		...
		try {
			...
			// Finally, we set the protocols.mission in the center UAV. As this is simulation, position and ID always match.
	...
	public boolean sendInitialConfiguration(int numUAV) {
		// No need of specific configuration, as the protocols.mission is automatically loaded
		return true;
	}
