...
public class MuscopSimProperties {
    ...
    public static long MISSION_TIMEOUT = 2000; 		// (ms) Timeout to wait after the protocols.mission is received and the master UAV changes its state
    ...
    // (rad) Master UAV yaw or heading (first protocols.mission segment orientation in simulation).
    public static volatile double formationYaw;
    // Mission sent from master to slaves (center protocols.mission).
    public static AtomicReference<Location3DUTM[]> missionSent = new AtomicReference<>();// Array containing the protocols.mission sent to the slaves
    ...
    public void storeMissionFile(List<File> selection) {
        ...
        if (missions == null) {
            ...
            // The master is assigned the first protocols.mission in the list
