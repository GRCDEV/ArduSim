...
class CommLinkObjectSimulationTest {
    ...
    void initCollisionAndCommunication(int role,boolean expectNull){
        ...
        int numUAV = 5;
        CommLinkObjectSimulation.init(numUAV,true,true,100);
        if(expectNull){
            assertNull(UAVParam.distances);
            assertNull(CommLinkObjectSimulation.isInRange);
        }else {
            assertEquals(UAVParam.distances.length, numUAV);
            assertEquals(CommLinkObjectSimulation.isInRange.length, numUAV);
        }
    }
